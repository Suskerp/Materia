    ...
    `}_sun(){const t=this.hass.states[this.config.sun_entity??"sun.sun"];if(!t)return V;const e=t.attributes?.next_rising,i=t.attributes?.next_setting;if(!e||!i)return V;const o=this.hass?.locale?.language||navigator.language||"en",s=t=>new Date(t).toLocaleTimeString(o,{hour:"numeric",minute:"2-digit"}),n=t=>t.getHours()+t.getMinutes()/60,a=n(new Date(e)),r=n(new Date(i)),l=n(new Date),c=(r-a+24)%24||12,d=24-c,h=24,p=t=>t/24*100,u=t=>{const e=(t-a+24)%24;return e<=c?h-17*Math.sin(Math.PI*e/c):h+9*Math.sin(Math.PI*(e-c)/d)},m=(t,e)=>{const i=[];for(let o=t;o<e;o+=.25)i.push(`${p(o).toFixed(2)} ${u(o).toFixed(2)}`);return i.push(`${p(e).toFixed(2)} ${u(e).toFixed(2)}`),i.join(" L")},g=`M${p(a).toFixed(2)} 24 L${m(a,r)} Z`,f=a>.01?`M0 24 L${m(0,a)} Z`:"",_=r<23.99?`M${p(r).toFixed(2)} 24 L${m(r,24)} L100 24 Z`:"",v=(l-a+24)%24<=c,b=p(l),y=u(l),x={new_moon:0,waxing_crescent:.125,first_quarter:.25,waxing_gibbous:.375,full_moon:.5,waning_gibbous:.625,last_quarter:.75,waning_crescent:.875},w=this.config.moon_entity?this.hass.states[this.config.moon_entity]:null,$=w&&w.state in x?x[w.state]:.5;return I`
      <div class="rect-tile sun">
        ...
        <div class="sun-times">
          ...
          ${w?I`<div class="moon-row"><ha-icon icon="mdi:moon-waning-crescent"></ha-icon> ${this.hass.formatEntityState?.(w)??w.state}</div>`:""}
        </div>
      </div>
