...
`];customElements.define("materia-weather-editor",class extends Tt{_formData(){return{show_temperature:!0,...this._config}}get _sections(){return[{title:"Content",icon:"mdi:card-text-outline",fields:[{name:"entity",required:!0,selector:{entity:{domain:"weather"}}},{name:"name",template:!0,selector:{text:{}}},{name:"icon",template:!0,selector:{icon:{}},context:{icon_entity:"entity"}}]},{title:"Sensors",icon:"mdi:water-percent",fields:[{name:"show_temperature",label:"Show temperature",selector:{boolean:{}}},{name:"temperature_entity",label:"Temperature sensor (optional)",selector:{entity:{domain:"sensor",device_class:"temperature"}}},{name:"humidity_entity",label:"Humidity sensor",selector:{entity:{domain:"sensor"}}}]},{title:"Actions",icon:"mdi:gesture-tap",fields:[{name:"tap_action",selector:{ui_action:{default_action:"more-info"}}}]}]}});const It={sunny:"m3o:sunny",clear:"m3o:sunny","clear-night":"mdi:weather-night",partlycloudy:"m3o:partly-cloudy-day",partly_cloudy:"m3o:partly-cloudy-day",cloudy:"m3o:cloud",rainy:"m3o:rainy",pouring:"m3o:rainy",snowy:"mdi:weather-snowy",fog:"m3o:foggy",windy:"mdi:weather-windy",lightning:"mdi:weather-lightning","lightning-rainy":"mdi:weather-lightning-rainy",hail:"mdi:weather-hail",exceptional:"mdi:alert-circle-outline"},Ht={"clear-night":"Clear night",partlycloudy:"Partly cloudy","lightning-rainy":"Thunderstorm","snowy-rainy":"Sleet",exceptional:"Exceptional"};class Wt extends(mt(ct)){static properties={hass:{attribute:!1},config:{state:!0},_resolvedIcon:{state:!0},_resolvedName:{state:!0}};static getConfigElement(){return document.createElement("materia-weather-editor")}static getStubConfig(t){const e=Object.keys(t?.states||{}).find(t=>t.startsWith("weather."))||"";return{entity:e}}static styles=Lt;setConfig(t){if(!t.entity)throw new Error("entity is required");this.config={...t}}updated(t){t.has("hass")&&this.hass&&(this._resolveField("icon","_resolvedIcon"),this._resolveField("name","_resolvedName"))}render(){if(!this.hass||!this.config)return I``;const t=this.hass.states[this.config.entity],e=this._isUnavailable(t),i=t?.state??"",o=!1!==this.config.show_temperature;let s=t?.attributes?.temperature,n=t?.attributes?.temperature_unit||"°";if(this.config.temperature_entity){const t=this.hass.states[this.config.temperature_entity];t&&(s=t.state,n=t.attributes?.unit_of_measurement||n)}const a=this._isTemplate(this.config.icon)?this._resolvedIcon:this.config.icon||It[i]||"mdi:weather-partly-cloudy";let r=null;if(this.config.humidity_entity){const t=this.hass.states[this.config.humidity_entity];t&&(r=t.state)}null==r&&null!=t?.attributes?.humidity&&(r=t.attributes.humidity);const l=Ht[i]||this._capitalize(i.replace(/-|_/g," ")),c=this._isTemplate(this.config.name)?this._resolvedName:this.config.name,d=o&&null!=s?`${s}${n}`:null;let h;h=e?"Unavailable":c||(d||(l||"—"));const p=[];e||(d&&h!==d&&p.push(d),h!==l&&p.push(l),null!=r&&p.push(`${r}%`));const u=p.join(" · ");return I`
    ...
    ${t.map(t=>H`<circle cx=${t} cy=${e} r="1.2" />`)}
      ...
      <line x1="6" y1="17" x2="18" y2="17" /><line x1="7.5" y1="20" x2="16.5" y2="20" /></g>`,hail:t=>H`${te(12,10,1,t.cloudDk)}${ie([8,12,16],18)}`,lightning:t=>H`${te(12,10,1,t.cloudDk)}<path d="M12 14 l-2.5 5 h2 l-1 4 4.5-6.5 h-2.2 l1.5-2.5 z" fill=${Kt} />`,"lightning-rainy":t=>H`${te(12,9.5,1,t.cloudDk)}${ee(Yt,[8,16],17)}<path d="M12 14 l-2 4 h1.8 l-0.8 3.5 4-5.5 h-2 l1.3-2 z" fill=${Kt} />`,windy:()=>H`<g stroke=${Xt} stroke-width="2" stroke-linecap="round" fill="none">
    ...
    <radialGradient id="wxMoonG-${t}" cx="35%" cy="28%" r="85%">
      <stop offset="0%" stop-color="color-mix(in srgb, ${Zt} 45%, #FFFFFF)" />
      <stop offset="60%" stop-color=${Zt} />
      <stop offset="100%" stop-color="color-mix(in srgb, ${Zt} 62%, #4A5AB8)" />
    </radialGradient>
...
`,me=[_t,gt,ft,re,ue,n`
    ...
    `}_convertWind(t,e,i){const o={"km/h":1,"m/s":3.6,mph:1.609344,kn:1.852,knots:1.852,"ft/s":1.09728};if(!i||i===e)return{v:t,u:e};const s=t*(o[e]??1);if("bft"===i){let t=[1,5,11,19,28,38,49,61,74,88,102,117].findIndex(t=>s<t);return-1===t&&(t=12),{v:t,u:"Bft"}}return{v:s/(o[i]??1),u:i}}_wind(){const t=this._value("wind_speed");if(null==t)return V;const e=this.config.sensor?this.hass.states[this.config.sensor]?.attributes?.unit_of_measurement??"km/h":this._weatherAttr("wind_speed_unit")??"km/h",{v:i,u:o}=this._convertWind(t,e,this.config.unit);let s=this.config.bearing_entity?this._numRaw(this.hass.states[this.config.bearing_entity]?.state):this._numRaw(this._weatherAttr("wind_bearing"));const n=null!=s?`${this.config.from_label??"From"} ${a=s,["N","NNE","NE","ENE","E","ESE","SE","SSE","S","SSW","SW","WSW","W","WNW","NW","NNW"][Math.round((a%360+360)%360/22.5)%16]}`:"";var a;const r=(null!=s?(s+180)%360:0)*Math.PI/180;return I`
      <div class="rect-tile clip wind">
        <svg class="blob-bg" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet">
          <path d=${we(50,50,44,r)} class="blob-fill" />
        </svg>
        <div class="overlay">
    ...
    `}_pressure(){const t=this._value("pressure");if(null==t)return V;const e=this.config.unit??this._weatherAttr("pressure_unit")??"hPa",i=this.config.min??("hPa"===e?950:28),o=this.config.max??("hPa"===e?1050:31),s=Math.min(1,Math.max(0,(t-i)/(o-i))),n=270*s-135,a=this.hass?.locale?.language||navigator.language||"en",r="hPa"===e?Math.round(t).toLocaleString(a):t;return I`
      <div class="shape-tile">
        <svg class="shape" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet">
          ...
          <path d=${$e(50,52,37.5,-135,135)} class="gauge-track thin" />
          ${s>.01?H`<path d=${$e(50,52,37.5,-135,n)} class="gauge-fill thin" />`:""}
        </svg>
        <div class="overlay">
          ...
          ${o.map(t=>I`
              <div class="gauge">
                <svg viewBox="0 0 100 86">
                  <path d=${$e(50,50,40,-135,135)} class="gauge-track" />
                  ${t.frac>.01?H`<path d=${$e(50,50,40,-135,270*t.frac-135)} class="gauge-fill" style="stroke:${t.color||"var(--wm-accent, #7bc96a)"}" />`:""}
                </svg>
  ...
  `],We=[{value:"auto",label:"Auto (from device class)"},{value:"percent",label:"Percent (filling cookie)"},{value:"temperature",label:"Temperature (thermometer)"},{value:"power",label:"Power (load bars)"},{value:"energy",label:"Energy"},{value:"binary",label:"On/off (spinning star)"},{value:"plain",label:"Plain value"}];customElements.define("materia-glance-tile-editor",class extends Tt{_formData(){return{variant:"auto",...this._config}}_sectionsSignature(){return this._config?.variant||""}get _sections(){const t=this._config?.variant,e={title:"Content",icon:"mdi:card-text-outline",fields:[{name:"entity",required:!0,selector:{entity:{}}},{name:"variant",label:"Visualization",selector:{select:{mode:"dropdown",options:We}}},{name:"name",label:"Title",selector:{text:{}}},{name:"icon",label:"Icon (overrides entity icon)",selector:{icon:{}}},{name:"label",label:"Subtitle",selector:{text:{}}}]},i={title:"Options",icon:"mdi:tune",fields:[]};"temperature"===t&&i.fields.push({name:"min",label:"Scale min (default 10°)",selector:{number:{mode:"box"}}},{name:"max",label:"Scale max (default 30°)",selector:{number:{mode:"box"}}}),"power"===t&&i.fields.push({name:"max",label:"Full-load watts (default 3000)",selector:{number:{mode:"box"}}});const o={title:"Appearance",icon:"mdi:palette-outline",fields:[{name:"accent",label:"Accent color (fill / bars / star)",color:!0,selector:{text:{}}},{name:"color",label:"Tile color",color:!0,template:!0,selector:{text:{}}},{name:"color_on",label:"Text color",color:!0,template:!0,selector:{text:{}}}]},s={title:"Actions",icon:"mdi:gesture-tap",fields:[{name:"tap_action",selector:{ui_action:{default_action:"more-info"}}}]};return i.fields.length?[e,i,o,s]:[e,o,s]}});const Ve="var(--md-sys-cust-color-weather-rain, #5fa8f5)",Ge="var(--md-sys-cust-color-scale-green, #5E9E50)",Xe="var(--md-sys-cust-color-scale-orange, #D9713C)",Ye="var(--md-sys-cust-color-scale-red, #C94D42)",Ke=["on","open","running","playing","heat","heating","home","true","active"];class Ze extends(mt(ct)){static properties={hass:{attribute:!1},config:{state:!0},_resolvedColor:{state:!0},_resolvedColorOn:{state:!0}};static styles=He;static getConfigElement(){return document.createElement("materia-glance-tile-editor")}static getStubConfig(t){const e=Object.keys(t?.states||{}).find(t=>t.startsWith("sensor."))||"";return{entity:e}}setConfig(t){if(!t.entity)throw new Error("Materia Glance Tile: entity is required");this.config={...t}}updated(t){t.has("hass")&&this.hass&&(this._resolveField("color","_resolvedColor"),this._resolveField("color_on","_resolvedColorOn"))}get _stateObj(){return this.hass?.states[this.config.entity]}_num(t){if(null==t||""===t||"unknown"===t||"unavailable"===t)return null;const e=Number(t);return Number.isFinite(e)?e:null}get _unit(){return this.config.unit??this._stateObj?.attributes?.unit_of_measurement??""}get _name(){return this.config.name??this._stateObj?.attributes?.friendly_name??this.config.entity}_icon(t){return this.config.icon||this._stateObj?.attributes?.icon||t}get _variant(){if(this.config.variant&&"auto"!==this.config.variant)return this.config.variant;const t=this._stateObj;if(!t)return"plain";const e=this.config.entity.split(".")[0],i=t.attributes.device_class,o=t.attributes.unit_of_measurement;return"binary_sensor"!==e&&"switch"!==e&&!Ke.includes(t.state)&&"off"!==t.state||null!=this._num(t.state)?"%"===o||"humidity"===i||"battery"===i||"moisture"===i?"percent":"temperature"===i||"°C"===o||"°F"===o?"temperature":"power"===i||"W"===o||"kW"===o?"power":"energy"===i||"kWh"===o||"Wh"===o||"MWh"===o?"energy":"plain":"binary"}_fmtState(){const t=this._stateObj;return this.hass.formatEntityState?.(t)??t.state}render(){if(!this.hass||!this.config)return I``;const t=this._stateObj;if(!t||this._isUnavailable(t))return I`<ha-card><div class="rect-tile unavailable">
    ...
    `}_binary(){const t=Ke.includes(this._stateObj.state),e=be(50,50,46,{vertices:8,innerRadius:.8,rounding:.15,rotate:-Math.PI/2});return I`
