...
`,di=[Ae,Se,Ee,ge,n`
  ...
  `],$i=e=>class extends e{willUpdate(e){super.willUpdate?.(e);const t="sidekick"===this.config?.variant?"sidekick":null;t?this.setAttribute("variant",t):this.removeAttribute("variant")}get _sidekick(){return"sidekick"===this.config?.variant}_shellPair(e){return this._sidekick?wi.sidekick:e?wi.active:wi.rest}_alertList(){return Array.isArray(this.config.alerts)?this.config.alerts:this.config.alert?[this.config.alert]:[]}_idle(e){return["off","idle","unknown","unavailable","false","0","none","","ok","docked"].includes(String(e??"").toLowerCase())}_alertText(e,t){if(null==t.text)return"";const i=this._isTemplate(t.text)?this._tplResults?.[`alertText${e}`]:t.text;return null==i?"":String(i).trim()}get _activeAlert(){const e=this._alertList();for(let t=0;t<e.length;t++){const i=e[t],s=this._alertText(t,i);if(i.entity){const e=this.hass?.states[i.entity];if(!e)continue;const t=String(e.state);if(null!=i.state){if(!(Array.isArray(i.state)?i.state.map(String):[String(i.state)]).includes(t))continue}else if(this._idle(t))continue;return{...i,text:s||(this.hass.formatEntityState?.(e)??t)}}if(s)return{...i,text:s}}return null}_resolveAlertTemplates(){this.config&&this._alertList().forEach((e,t)=>{null!=e.text&&this._resolveTemplateValue(`alertText${t}`,e.text)})}_renderBurst({alarm:e=!1,working:t=!1}={}){return!1===this.config.burst?W:I`
    ...
    `}getGridOptions(){return{columns:12,rows:"auto"}}getCardSize(){return 4}}customElements.define("materia-vacuum-hero",Ni),window.customCards=window.customCards||[],window.customCards.push({type:"materia-vacuum-hero",name:"Materia Vacuum Hero",description:"Robot-vacuum headline — derived ETA, negated working states, and mop/consumable warnings. Roborock and Ecovacs.",preview:!0});const Bi=[Ae,Se,ge,n`
    ha-card {
      background: transparent;
      box-shadow: none;
      border: none;
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const Bs=Cs(class extends Ss{constructor(){super(...arguments),this.key=W}render(e,t){return this.key=e,t}update(e,[t,i]){return t!==this.key&&(((e,t=ks)=>{e._$AH=t})(e),this.key=t),i}}),js=[Ae,Se,Ee,ge,n`
    ha-card {
      background: transparent;
      box-shadow: none;
      border: none;
        ...
        <div class="sheet">
          <div class="echo">
            ...
            ${(()=>{const i=this._hasSelection?t.head:e??t.head,s=this._hasSelection?t.sub:e?this.config.pending_sub??we("sched_pending_sub",this.hass):t.sub;return I`
                ${Bs(i,I`<span class="headline swap">${i}</span>`)}
                ${Bs(s,I`<span class="subline swap">${s}</span>`)}
              `})()}
          </div>
